...
def require_http_methods(request_method_list: List[str]) -> Callable[[Func], Func]:
    def decorator(func: Func) -> Func:
        @ft.wraps(func)
        def inner(
            self: "JsonRPCSite", request: HttpRequest, *args: Any, **kwargs: Any
        ) -> Any:
...
class JsonRPCSite:
    def __init__(
        self, path: str = "", spec: Optional[specs.Specification] = None, **kwargs: Any
    ):
        self._path = path
        self._spec = spec
        ...
        if self._spec:
            self._urls.append(
                urls.path(
                    utils.join_path(self._path, self._spec.path), self._generate_spec
                )
            )
            ...
            if self._spec.ui and self._spec.ui_path:
                ...
                self._urls.extend(
                    (
                        ...
                        urls.path(
                            utils.join_path(path, "index.html"), self._ui_index_page
                        ),
                    )
                )
                self._urls.extend(
                    static(path, document_root=str(self._spec.ui.get_static_folder()))
                )
    ...
    def _generate_spec(self, request: HttpRequest) -> HttpResponse:
        ...
        schema = self._spec.schema(
            path=endpoint_path, methods_map={"": self._dispatcher.registry.values()}
        )
    ...
    def _rpc_handle(self, request: HttpRequest) -> HttpResponse:
        ...
        if response is None:
            ...
            return HttpResponse(
                response_text, content_type=pjrpc.common.DEFAULT_CONTENT_TYPE
            )
    ...
    def _setup(self) -> None:
        ...
        for path, endpoint in getattr(settings, "JSONRPC_ENDPOINTS", {}).items():
            json_encoder = import_string(
                endpoint.get("JSON_ENCODER", "pjrpc.server.dispatcher.JSONEncoder")
            )
            json_decoder = import_string(
                endpoint.get("JSON_DECODER", "json.JSONDecoder")
            )
            spec = (
                import_string(endpoint["SPEC"]) if endpoint.get("SPEC", None) else None
            )
            middlewares = [
                import_string(middleware)
                for middleware in endpoint.get("MIDDLEWARES", [])
            ]
            error_handlers = {
