 ...
 */
export type ErrorResponse = InvalidDataResponse | NotAllowedResponse | NotFoundResponse | RuleViolationResponse | InternalErrorResponse | BadRequestResponse;
...
export class InvalidData extends BaseSerializableError<InvalidDataResponse> {
    ...
    static fromZodError(
        ...
        return new InvalidData({ errors: grouped, data: item as JSONable, status, message });
    }
...
export class NotFound extends BaseSerializableError<NotFoundResponse> {
    ...
    constructor(args: {
        ...
        const { resource, message = "Resource not found", id = "unknown", context } = args;
 ...
 */
function validationErrorCodeFromZodIssue(issue: z.core.$ZodIssue): ValidationErrorCode {
    switch (issue.code) {
        case "invalid_type":
...
export function responseFromException(error: unknown): ErrorResponse {
    if (error instanceof RuleViolation && env.PRODUCTION) return error.toInternalErrorResponse();
